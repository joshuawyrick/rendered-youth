...
const COLORS = ['Black', 'White', 'Navy', 'Gray', 'Red', 'Green', 'Blue', 'Purple'];
  ...
  const fetchProductVariants = async () => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        // Initialize with default variants if none exist
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-6">
          ...
          {/* Product Variants */}
          <RYCard className="p-4">
            <h3 className="text-lg font-semibold mb-4">Size & Color Variants</h3>
            <div className="max-h-96 overflow-y-auto">
              <div className="grid gap-2">
                <div className="grid grid-cols-6 gap-2 items-center p-2 bg-gray-50 rounded font-medium">
                  <div>Size</div>
                  <div>Color</div>
                  <div>Price Adj. ($)</div>
                  <div>Final Price</div>
                ...
                {variants.map((variant, index) => (
                  <div key={`${variant.size}-${variant.color}`} className="grid grid-cols-6 gap-2 items-center p-2 border rounded">
